...
export default function Dashboard() {
  ...
  const optionEmailCompany = lead.map((leads) => ({
    value: leads.nome,
    label: leads.nome,
    name: "EmailCompany",
  }));
  ...
  const handleRequestGet = (event: any) => {
    if (event) {
      fetch(
        `https://ystembrutamarketing.com.br:21160/lead?${event.name}=${event.value}`
      )
  ...
  return (
    <>
      <header className={styles.header}>
        <Image src={Logo} alt="Logo Bruta Marketing" width={100} />
        <a href="">Sair</a>
      </header>
        ...
        <article className={styles.main}>
          ...
          <div className={styles.maiores}>
            ...
            <CreatableSelect
              ...
              className={styles.inputNome}
              classNamePrefix="select"
              options={optionEmailCompany}
              onChange={handleRequestGet}
              placeholder="Email do Company"
            />
            <CreatableSelect
              id="NomeEmpresa"
              isClearable
